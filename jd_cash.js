...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `https://ntent.com/l499477004/updateTeam/master/jd_cashShareCode.json`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function getAuthorShareCode() {
  return new Promise(resolve => {
    $.get({url: "https://gocks/updateTeam/raw/main/jd_cash.json",headers:{
